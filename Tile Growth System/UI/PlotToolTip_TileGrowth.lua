...
function FetchData(pPlot)
	...
	for k,v in pairs(AllProperties) do
		table.insert(data.PlotProperties, ExposedMembers.TileGrowth.UI_GetData(k, pPlot))
	end
...
function GetDetails(data)
	...
	for k,v in pairs(data.PlotProperties) do
		...
		local percentUtil = math.floor((v[1]*100)/v[3])
		local percentGrowth = math.floor((v[2]*100)/v[3])
