...
function DefineGrowthProperty(NewPropertyArgs : PropertyListData)
    ...
    if NewPropertyArgs ~= nil then
        growthCalcFunc = NewPropertyArgs.GrowthFunc
        if growthCalcFunc == nil then
            NewPropertyArgs.GrowthFunc = function()
...
function InitPlotPropertyData(PropertyID, pPlot)
    if pPlot ~= nil then
        ...
        WriteGrowth(PropertyID, pPlot, 0)
...
function CheckTrigger(NewVal, OldVal, Threshold)
    ...
    if OldVal < Threshold & NewVal >= Threshold then
        triggered = true
        direction = 1
    elseif OldVal >= Threshold & NewVal < Threshold then
        triggered = true
        direction = -1
    end
