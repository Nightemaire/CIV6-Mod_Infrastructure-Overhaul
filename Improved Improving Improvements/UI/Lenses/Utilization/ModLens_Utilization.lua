...
local VillageColor			= UI.GetColorValue("COLOR_UTIL_VILLAGE")
local TownshipColor			= UI.GetColorValue("COLOR_UTIL_TOWNSHIP")
        ...
        if localPlayerVis:IsRevealed(plot:GetX(), plot:GetY()) then
            if plot:GetOwner() == localPlayer then
                if plot:IsCity() then
					...
					local growth = plot:GetProperty("PLOT_GROWTH")
					if growth == nil then 
						growth = 0
					else
...
UtilizationLensLegend = {
    ...
    {"LOC_TOOLTIP_UTILIZATION_VILLAGE",			VillageColor},
	{"LOC_TOOLTIP_UTILIZATION_TOWNSHIP",		TownshipColor},
